...
import { Star, StarBorder, Check, RateReview, Add } from '@mui/icons-material';
import { useMutation, useQueryClient, useQuery } from '@tanstack/react-query';
import AddQuestion from './AddQuestion';
import CustomModal from '../../../components/base/customModal';
import getQuestions from '../services/getQuestions';
import updateQuestion, {
  UpdateQuestionInputProps,
} from '../services/updateQuestion.ts';
  ...
  const { mutate: mutateStatus } = useMutation({
    ...
    onSuccess: (response) => {
      ...
      queryClient.setQueryData(
        [queryKeys.LIST_QUESTIONS, listID],
        (oldData: any) => {
          if (!oldData) return;
    ...
    onSuccess: (response) => {
      ...
      queryClient.setQueryData(
        ...
        (oldData: any) => {
          ...
          return {
            ...oldData,
            data: { ...oldData.data, questions: updatedQuestions },
          };
        },
      );
    },
        ...
        {selectedQuestions.length > 0 && (
          <Box display="flex" gap={2}>
            <Button
              ...
              onClick={() =>
                handleBulkAction(
                  allMarkedAs('important') ? 'unimportant' : 'important',
                )
              }
            >
              {allMarkedAs('important')
                ? 'Unmark as Important'
                : 'Mark as Important'}
            </Button>
            <Button
              ...
              color="secondary"
              onClick={() =>
                handleBulkAction(allMarkedAs('review') ? 'unreview' : 'review')
              }
            >
            ...
            <Button
              variant="contained"
              onClick={() =>
                handleBulkAction(allMarkedAs('done') ? 'undone' : 'done')
              }
            >
      ...
      {!isLoading && !isError && (
        <>
                  ...
                  paginatedQuestions.map((q) => (
                    <TableRow key={q.questionId}>
                      ...
                      <TableCell align="center">
                        <Box
                          onClick={() =>
                            handleToggleStatus(q.questionId, 'done', q.done)
                          }
                          sx={{
                            cursor: 'pointer',
                      ...
                      <TableCell align="center">
                        <IconButton
                          onClick={() =>
                            handleToggleStatus(q.questionId, 'review', q.review)
                          }
          ...
          {questions.length > 5 && (
            ...
            >
              <Typography>
                Page: {page} of {Math.ceil(questions.length / limit) || 1}
              </Typography>
              <Select
                value={limit}
