...
const ForgotPassword = () => {
  ...
  const validateStep3 = () => {
    ...
    // Create an object that includes all fields of the errors state
    let newErrors = { email: '', otp: '', password: '', confirmPassword: '' };
    ...
    setErrors(newErrors); // Set all errors, including the email and otp fields
    return isValid;
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    else if (step === 'enterOtp') verifyOtp();
  ...
  return (
    <div className="min-h-screen flex px-[10%]">
      ...
      <div className="hidden md:flex w-1/2 text-white flex-col p-10 bg-[url('/bgsvg.svg')] bg-no-repeat bg-contain bg-center">
        <h1 className="font-protest font-bold text-6xl mb-10 text-blue-400 p-10">KODACK</h1>
      </div>
      ...
      <div className="flex w-full md:w-1/2 justify-center items-center">
        <form
          ...
          style={{ border: '1px solid rgba(51, 60, 77, 0.6)', background: 'rgba(255, 255, 255, 0.02)' }}
          onSubmit={handleSubmit}
        >
          {step === 'enterEmail' && (
            <>
              <Typography variant="h5" component="h1" className="text-white mb-4">Forgot Password</Typography>
              <TextField
                label="Email"
                variant="outlined"
                error={!!errors.email}
              ...
              />
              <Button fullWidth variant="contained" color="primary" type="submit" disabled={isActionDisabled || sendOtpStatus === 'pending'}>
                {sendOtpStatus === 'pending' ? <CircularProgress size={20} /> : 'Send OTP'}
              </Button>
            </>
          )}
          ...
          {step === 'enterOtp' && (
            <>
              <Typography variant="h5" component="h1" className="text-white mb-4">Enter OTP</Typography>
              <TextField
                label="OTP"
                variant="outlined"
                fullWidth
                ...
                sx={defaultStyles.inputStyles}
              />
              <Button fullWidth variant="contained" color="primary" type="submit" disabled={isActionDisabled || verifyOtpStatus === 'pending'}>
                {verifyOtpStatus === 'pending' ? <CircularProgress size={20} /> : 'Verify OTP'}
              </Button>
          ...
          {step === 'enterNewPassword' && (
            <>
              <Typography variant="h5" component="h1" className="text-white mb-4">Reset Password</Typography>
              <FormControl fullWidth variant="outlined" sx={defaultStyles.inputStyles}>
                ...
                >
                  {resetPasswordStatus === 'pending' ? <CircularProgress size={20} /> : 'Reset Password'}
                </Button>
              </>
            )}
          </form>
